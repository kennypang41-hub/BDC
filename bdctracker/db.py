...
def load_positions(conn: sqlite3.Connection, positions: Iterable[Position]) -> dict:
    ...
    for p in positions:
        ...
        mark_rows.append(
            (
                ...
                _iso(p.maturity_date), _iso(p.acquisition_date),
                p.year_acquired, p.year_matures, p.industry, p.country,
            p.principal_currency, p.fair_value_currency,
            p.principal_usd, p.fx_rate, p.fx_date, p.fair_value_level,
    ...
    conn.executemany(
        ...
        INSERT INTO marks (
            ...
            maturity_date, acquisition_date, year_acquired, year_matures,
            industry, country,
            principal_ccy, fair_value_ccy, principal_usd, fx_rate, fx_date,
            fair_value_level, is_non_accrual,
        ...
        ON CONFLICT(loan_id, period_end) DO UPDATE SET
            ...
            year_acquired=COALESCE(excluded.year_acquired, marks.year_acquired),
            year_matures=COALESCE(excluded.year_matures, marks.year_matures),
            ...
            principal_usd=COALESCE(excluded.principal_usd, marks.principal_usd),
            fx_rate=COALESCE(excluded.fx_rate, marks.fx_rate),
            fx_date=COALESCE(excluded.fx_date, marks.fx_date),
